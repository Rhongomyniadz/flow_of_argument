...
您说：
now I want to plot the individual assumptions separately instead of doing the average, come up with a better visualization
...
#!/usr/bin/env python3
"""
Assumption-Level Visualizer — no averaging, per-assumption views (single file)

What this adds (beyond earlier graphs):
- PER-ASSUMPTION TIMELINE with cross-speaker links:
  For each episode, choose the top-2 speakers by speaker_id and plot every individual
  assumption as a dot on a two-lane timeline (speaker A on top, speaker B on bottom).
  Draw lines connecting semantically similar assumptions across speakers (based on cosine
  similarity of per-assumption embeddings). This replaces the averaged host/guest lines.

- PER-ASSUMPTION MAP (t-SNE):
  For each episode, embed every assumption, project to 2D t-SNE, and color by speaker_id.
  Optionally highlights assumptions that have a strong cross-speaker match.

Inputs (same JSON as before, e.g. under results/covid):
[
  {
    "turn_text": "...",
    "speaker_id": ["SPEAKER_01"] | "SPEAKER_01" | {"id": "..."} | null,
    "inferred_speaker_name": "Alice" | "NO_INFERRED_SPEAKER",
    "inferred_speaker_role": "...",  # ignored for per-assumption plots
    "assumptions": ["...", "..."]
  },
  ...
]
...
Outputs (in --out_dir):
- assumptions_timeline/<episode>_assumption_timeline.png
- assumptions_tsne/<episode>_assumption_tsne.png

Dependencies:
  pip install numpy pandas scikit-learn networkx matplotlib sentence-transformers
"""
