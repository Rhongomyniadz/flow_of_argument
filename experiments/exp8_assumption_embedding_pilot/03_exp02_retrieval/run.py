...
def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(description="Evaluate frozen next-turn retrieval.")
...
def run(args: argparse.Namespace) -> None:
    ...
    rows: list[dict[str, Any]] = []
    for anchor in tqdm(anchors, desc="stage 03 retrieval", unit="anchor", dynamic_ncols=True):
    ...
    pd.DataFrame(rows).to_csv(args.output_dir / "rows.csv", index=False)
