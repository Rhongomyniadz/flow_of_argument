...
#SBATCH --output=_log/exp8_embed_submit_%j.out
...
if [[ "${MODE}" == "worker" ]]; then
  "${PYTHON_BIN}" -m experiments.exp8_assumption_embedding_pilot.prepare.cache_embeddings --mode worker "${base_args[@]}" --patch-index "${SLURM_ARRAY_TASK_ID:-${PATCH_INDEX:-0}}" --num-patches "${NUM_PATCHES:?NUM_PATCHES is required}"
  exit
fi
if [[ "${MODE}" == "merge" ]]; then
  "${PYTHON_BIN}" -m experiments.exp8_assumption_embedding_pilot.prepare.cache_embeddings --mode merge "${base_args[@]}" --num-patches "${NUM_PATCHES:?NUM_PATCHES is required}"
  exit
fi
[[ "${MODE}" == "submit" ]] || { echo "MODE must be submit, worker, or merge" >&2; exit 2; }
count_output="$("${PYTHON_BIN}" -m experiments.exp8_assumption_embedding_pilot.prepare.cache_embeddings --mode count "${base_args[@]}")"
...
worker_command=(sbatch --parsable --job-name=exp8_embed --partition="${GPU_PARTITION}" --time=05:45:00 --cpus-per-task=4 --mem=64G "${gres[@]}" --array="0-$((num_patches-1))%${MAX_CONCURRENT_TASKS}" --output="${SCRIPT_DIR}/_log/embed_%A_%a.out" "${dependency[@]}" --export="ALL,MODE=worker,NUM_PATCHES=${num_patches}" "${SELF}")
...
merge_job_id="$(sbatch --parsable --job-name=exp8_embed_merge --partition="${CPU_PARTITION}" --time=01:00:00 --cpus-per-task=4 --mem=16G --output="${SCRIPT_DIR}/_log/embed_merge_%j.out" --dependency="afterok:${array_job_id}" --export="ALL,MODE=merge,NUM_PATCHES=${num_patches}" "${SELF}")"
