...
#SBATCH --output=_log/exp8_exp05_submit_%j.out
...
if [[ -d "${PWD}/experiments/exp8_assumption_embedding_pilot" ]]; then REPO_ROOT="${PWD}"; SCRIPT_DIR="${REPO_ROOT}/experiments/exp8_assumption_embedding_pilot"; elif [[ -d "${PWD}/common" && -f "${PWD}/06_run_exp05_fusion.sh" ]]; then SCRIPT_DIR="${PWD}"; REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"; else SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"; REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"; fi
SELF="${SCRIPT_DIR}/06_run_exp05_fusion.sh"; cd "${REPO_ROOT}"; mkdir -p "${SCRIPT_DIR}/_log"
...
if [[ "${MODE}" == "worker" ]]; then "${PYTHON_BIN}" -m experiments.exp8_assumption_embedding_pilot.exp05_mini_fusion.run --mode worker "${args[@]}" --patch-index "${SLURM_ARRAY_TASK_ID:-${PATCH_INDEX:-0}}"; exit; fi
if [[ "${MODE}" == "merge" ]]; then "${PYTHON_BIN}" -m experiments.exp8_assumption_embedding_pilot.exp05_mini_fusion.run --mode merge "${args[@]}"; exit; fi
...
worker_command=(sbatch --parsable --job-name=exp8_exp05 --partition="${GPU_PARTITION}" --time=05:45:00 --cpus-per-task=4 --mem=64G "${gres[@]}" --array="0-8%${MAX_CONCURRENT_TASKS}" --output="${SCRIPT_DIR}/_log/exp05_%A_%a.out" "${dependency[@]}" --export=ALL,MODE=worker "${SELF}")
...
array_job_id="$("${worker_command[@]}")"; merge_job_id="$(sbatch --parsable --job-name=exp8_exp05_merge --partition="${CPU_PARTITION}" --time=01:00:00 --cpus-per-task=4 --mem=16G --output="${SCRIPT_DIR}/_log/exp05_merge_%j.out" --dependency="afterok:${array_job_id}" --export=ALL,MODE=merge "${SELF}")"
