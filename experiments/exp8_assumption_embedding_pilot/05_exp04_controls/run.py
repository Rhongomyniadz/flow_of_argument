...
def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(description="Evaluate counterfactual assumption controls.")
...
def run(args: argparse.Namespace) -> None:
    ...
    source_ids = {str(anchor["anchor_id"]) for anchor in anchors}
    rows: list[dict[str, Any]] = []
    ...
    for control_type in CONTROL_TYPES:
        ...
        for anchor in tqdm(
            anchors,
            desc=f"stage 05 {control_type}",
            unit="anchor",
            dynamic_ncols=True,
        ):
            donor_id = donor_map.get(str(anchor["anchor_id"]))
            if donor_id is None:
                continue
    ...
    if frame.empty:
        raise RuntimeError("Stage 05 produced no matched control examples")
    if frame.duplicated(["anchor_id", "control_type", "condition"]).any():
        raise RuntimeError("Duplicate anchor/control/condition rows")
    mixed_pools = frame.groupby("anchor_id")["candidate_pool_hash"].nunique()
    if (mixed_pools != 1).any():
        raise RuntimeError("Counterfactual controls changed the candidate pool")

    records = frame.to_dict("records")
    metrics: list[dict[str, Any]] = []
    deltas: list[dict[str, Any]] = []
    for control_type in CONTROL_TYPES:
        subset = [row for row in records if row["control_type"] == control_type]
