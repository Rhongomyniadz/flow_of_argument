...
The step-wise specification analysis fits eight nested models on that same sample to identify
which variable group changes the agreement and disagreement coefficients:

1. stance movement only;
2. lagged stance movement;
3. previous iceberg ratio;
4. linear and quadratic timeline position;
5. category fixed effects, completing the ratio baseline;
6. current-turn duration;
7. pre-turn gap and overlap; and
8. previous-turn duration.

Each stage contains every group from the preceding stage. The step-wise comparison reports
the coefficient change introduced by each group and flags the largest absolute incremental
change separately for agreement and disagreement.
...
- `rq1_stepwise_coefficients.csv`: all coefficients from the eight nested specifications.
- `rq1_stepwise_stance_comparison.csv`: stance estimates and incremental changes by stage.
- `rq1_stepwise_model_fit.csv`: fit statistics for all step-wise specifications.
...
- `rq1_stepwise_comparison.pdf` and `.png`: two-panel agreement/disagreement paths across
  every variable group.
